...
**This is a framework migration (DSPy → BAML on the bot side), not only a schema change.** The bot is currently `dspy.ChainOfThought(GillSignature)`; the contract lives in a docstring and the "output fields" are parsed-hopefully text. Moving to BAML means the DSPy module, its (never-wired) assertion machinery, the `ChainOfThought` reasoning field, and the `dspy.LM` config all move too — scoped as *replace the module*, not *add a schema to it*.

- Replace `GillSignature` docstring with field descriptions on `GillAnswer`. **The field descriptions are the load-bearing work** (E-12: bare typed slots collapsed informative refusals 0/24 — the slots carry no behavior; the descriptions do). A year of prose-prompt discipline (informative-vs-flat, anachronism disclaimer, bridge behavior, Zone-1 constraints) must be transferred into `@description`s local to each slot, likely with per-field few-shot. This is the "pre-draft during ingestion" task — do it in the ingestion window.
- **Port `reasoning` as the first-position field of `GillAnswer`** (not dropped with DSPy). It is a real product feature — surfaced on refusals with named SIDs rendered as amber pills ([`main.py`](../../backend/main.py) `SearchResponse.reasoning`) — and field order is BAML's equivalent of ChainOfThought's think-then-answer sequence. Verify N=8 (E-12 style) that reasoning-then-phrases ordering preserves answer quality rather than assuming it.
- **Preserve the `raw_answer` trace continuity.** The determinism harness ([`evals/determinism_harness.py`](../../evals/determinism_harness.py)) distinguishes "bot LLM produced different text" from "verifier repaired differently" via `stages_capture["bot_raw_answer"]`. Capture BAML's pre-parse raw model output into the same trace field or the harness goes blind across the migration.
- Rewrite `GroundedGillBot.forward()` to consume the Pydantic object instead of parsing prose. **This deletes, not ports, ~half the file:** `QUOTE_WITH_CITE_RE` + its 80-char-gap heuristic (quote↔SID pairing becomes sibling fields, unfakeable), `_strip_trailing_prose` (a synthesis closer after the last `phrases[]` entry is *unrepresentable*, not merely forbidden), the No-Free-Lunch keyword guard (→ typed `refusal.mode`), and the citation-extraction gymnastics (`extract_ids`, string-repr-of-list handling, `normalize_id_for_cmp`) that exist only because DSPy doesn't enforce structure.
- Rewrite the verifier as a per-field difflib loop
- Rewrite the gutter renderer to consume structured events
- **Retain the Zone-3 sweep + judge over the `framing`/`zone1_bridge` fields** — they are NOT retired by the schema. E-12 Q4 measured 5–8/8 Zone-3 leakage into free-text framing; structure shrinks the policed surface (25-word labelled strings vs 800-token prose) but does not eliminate the impulse. `_suppress_zone3` survives, now scoped to the framing fields.

**Do NOT hybridize (BAML-inside-DSPy).** Considered and rejected: it splits the prompt across two homes — DSPy owning orchestration + docstring, BAML owning the field descriptions that E-12 proved are load-bearing — which is the exact two-sided, one-instrumented boundary shape that has cost this project a week at every A→B seam it has audited. The only thing DSPy would contribute is a ~15-line `forward()` and access to optimizers this project's validation discipline has already effectively forbidden. Go full BAML; consolidate onto the already-instrumented model-callsite.

**Door deliberately closed (recorded so future-me knows it was a choice):** DSPy's prompt-optimization/compilation — teleprompters, bootstrapped few-shot, metric-driven compile — is being given up. Verified never used (no `teleprompt`/`BootstrapFewShot`/`dspy.Example`/`trainset`/`.compile()` anywhere in the repo); the GillSignature docstring was hand-tuned through eval-gated iteration against a metric (three-way judge + verifier + human read) that isn't the scalar DSPy optimizes. Automated prompt mutation was never going to pass this project's gates. Closing the door is a simplification, not a capability loss — but it is a choice, not an oversight.
