...
### The method, in miniature (the demotion chain)

Over this session a **model-choice question became a routing question became a residual handler** — because each deterministic layer added (the compositor's count, then the per-note crops) shrank what the stochastic layers had to be trusted with. That is the whole project's method compressed to one line: **don't pick the best guesser; keep shrinking what needs guessing.** The 8/8-vs-6/8-vs-3/8 segmentation table that consumed a week of bakeoffs was retired not by a better model but by a typographic habit in the left margin — "the pixels can erase the problem" was the hypothesis, 8/8-with-gemma-alone is the measurement.

And the symmetric slice is worth its own note, because what it did is rarer than validation: **it was designed to catch a false conclusion and instead upgraded a true one.** Gemma-primary crowned on qwen3.6's failure set would have been asymmetric evidence forever; gemma holding 50/50 on clean pages *and* winning Hebrew on the incumbent's own turf (p516/p925) is the full ledger. The guard against the handful-of-pages trap turned a hypothesis into a finding.
